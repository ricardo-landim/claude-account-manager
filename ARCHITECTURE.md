...
as access token, no refresh token and an expiry the CLI never reaches, so it never tries to
refresh. Measured before adopting it: a session with no environment variable and only that
